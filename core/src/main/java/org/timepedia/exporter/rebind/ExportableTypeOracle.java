...
public class ExportableTypeOracle {
  ...
  private final JClassType exportAllType;

  private final TypeOracle typeOracle;

  private final TreeLogger log;
  ...
  private final JClassType exportOverlayType;

  private final Map<String, JExportOverlayClassType> overlayTypes =
      new HashMap<String, JExportOverlayClassType>();

  // We maintain a cache with overlay classes exported via export closure
  static HashMap<String, JExportableClassType> closuresCache =
      new HashMap<String, JExportableClassType>();
  ...
  public ExportableTypeOracle(TypeOracle typeOracle, TreeLogger log) {
    ...
    for (JClassType t : typeOracle.getTypes()) {
      if (t.isAssignableTo(exportOverlayType) && !t.equals(exportOverlayType)) {
        ...
        overlayTypes.put(targetType.getQualifiedSourceName(), new JExportOverlayClassType(this, t));
      }
    }
  }
  ...
  public List<JClassType> findAllExportableTypes() {
    ...
    for (JClassType t : typeOracle.getTypes()) {
      if (types.contains(t) || t.equals(exportAllType) || t.equals(exportableType)
          || t.equals(exportOverlayType)) {
        continue;
      }
    ...
    try {
      ...
      if (type.isPrimitive() != null) {
        ...
        return new JExportOverlayClassType(this, type.isClassOrInterface());
          ...
          && (cType.isAssignableTo(exportableType) || cType.isAssignableTo(stringType)
              || cType.isAssignableTo(dateType) || cType.isAssignableTo(jsoType))) {
  ...
  public boolean isExportable(JAbstractMethod method, JExportableClassType type) {
    ...
    if (method.isPublic()) {
      Export e;
      if (method instanceof JConstructor && isInstantiable(method.getEnclosingType())
        ...
        export = true;
      } else if (type != null && (e = type.getType().getAnnotation(Export.class)) != null
          && e.all()) {
          ...
          for (JMethod m : c.getMethods()) {
            if (!isNotExportable(m.getAnnotation(NoExport.class))
              ...
              if (m.getReadableDeclaration(true, true, false, true, true).equals(
                  ((JMethod) method).getReadableDeclaration(true, true, false, true, true))) {
                export = true;
                break;
              }
            }
  ...
  public boolean isExportable(JClassType type) {
    return isExportable(type.getAnnotation(Export.class))
        || (type.isInterface() != null && isExportable(type.getAnnotation(ExportClosure.class)));
  }
