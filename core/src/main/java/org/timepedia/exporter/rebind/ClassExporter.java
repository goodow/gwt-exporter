...
public class ClassExporter {
  ...
  private final ArrayList<JExportableClassType> exported;

  private final HashSet<String> overloadExported = new HashSet<String>();
  ...
  public ClassExporter(TreeLogger logger, GeneratorContext ctx, HashSet<String> visited) {
    this.logger = logger;
    this.ctx = ctx;
  ...
  /**
   * This method generates an implementation class that implements Exporter and returns the fully
   * qualified name of the class.
   */
  public String exportClass(String requestedClass, boolean export) throws UnableToCompleteException {
    ...
    if (requestedType == null) {
      logger.log(TreeLogger.ERROR, "Type '" + requestedClass + "' does not implement Exportable",
          null);
    ...
    if (debug) {
      ...
      System.out.println("    getJSQualifiedExportName: "
          + requestedType.getJSQualifiedExportName());
      System.out.println("    getQualifiedExporterImplementationName: "
    ...
    String superClass =
        xTypeOracle.isStructuralType(requestedType.getType()) ? requestedClass : xTypeOracle
            .isExportAll(requestedClass) ? "ExporterUtil.ExportAll" : null;
    ...
    if (export && xTypeOracle.isExportAll(requestedClass)) {
      ...
      sw.println("\nif(pkg) for (p in pkg) if ($wnd." + requestedType.getJSQualifiedExportName()
          + "[p] === undefined) $wnd." + requestedType.getJSQualifiedExportName() + "[p] = pkg[p];");
  ...
  /**
   * This method generates an implementation of the specified interface that accepts a
   * JavaScriptObject in its constructor containing a callback. It then delegates the single-method
   * of the interface to this callback.
   * <p/>
   * For example:
   * <p/>
   * <p/>
   * / ** * @gwt.exportClosure * / public interface ClickListener implements Exportable { public
   * void onClick(Sender s); }
   * <p/>
   * generates a delegation class
   * <p/>
   * public class ClickListenerImpl implements Exporter, ClickListener {
   * <p/>
   * private JavaScriptObject jso; public ClickListenerClosure(JavaScriptObject jso) { this.jso =
   * jso; }
   * <p/>
   * public void onClick(Sender s) { invoke(jso, ExporterBase.wrap(s)); }
   * <p/>
   * public native void invoke(JavaScriptObject closure, JavascriptObject s) { closure(s); }
   * <p/>
   */
  public void exportClosure(JExportableClassType requestedType) throws UnableToCompleteException {
    ...
    sw.indent();
    sw.println("return obj != null && obj instanceof " + genName + " && jso.equals(((" + genName
        + ")obj).jso);");
    sw.outdent();
    sw.println("}");
    ...
    sw.println("public static " + genName + " makeClosure(" + ExportableTypeOracle.JSO_CLASS
        + " closure) {");
    sw.indent();
    ...
    for (JExportableMethod method : requestedType.getExportableMethods()) {
      ...
      if (retType != null && retType.needsExport()
      ...
      boolean isBoolean =
          !isVoid && retType != null && retType.getQualifiedSourceName().equals("boolean");
      boolean isPrimitive =
          !isVoid && retType != null && retType instanceof JExportablePrimitiveType;
      ...
      for (int i = 0, l = method.getExportableParameters().length; i < l; i++) {
        if (xTypeOracle
            .isJavaScriptObject(method.getExportableParameters()[i].getParam().getType())) {
          apply = ARG_PREFIX + i;
          break;
        }
      }
  ...
  /**
   * Get SourceWriter for following class and preamble package packageName; import
   * com.google.gwt.core.client.GWT; import org.timepedia.exporter.client.Exporter; public class
   * className implements interfaceName (usually Exporter) {
   * <p/>
   * }
   * 
  ...
  protected SourceWriter getSourceWriter(TreeLogger logger, GeneratorContext context,
      String packageName, String className, String superClass, String... interfaceNames) {
  ...
  private HashMap<String, DispatchTable> buildDispatchTableMap(JExportableClassType requestedType,
      boolean staticDispatch) throws UnableToCompleteException {
    ...
    for (JExportableMethod meth : requestedType.getExportableMethods()) {
      ...
      staticDispatch && !meth.isInStaticMap() || !staticDispatch && meth.isInStaticMap()) {
        continue;
      }
      ...
      if (!dt.addSignature(meth)) {
        logger.log(TreeLogger.ERROR, "Ambiguous method signature " + meth.getJSNIReference()
  ...
  /**
   * Create a static wrapper for a method which can not be exported in JSNI. Typically all methods
   * which have 'long' parameters or return 'long'
   * 
   ...
   * @param requestedType
   */
  private void createStaticWrapperMethod(JExportableMethod method,
      JExportableClassType requestedType) {
    String r =
        method.getExportableReturnType(false) != null ? method.getExportableReturnType(false)
    ...
    if (method.isStatic()) {
      ...
      function +=
          requestedType.getQualifiedSourceName() + " instance"
              + (method.getExportableParameters().length > 0 ? ", " : "");
      body += "instance";
    }
    body += "." + method.getName() + "(";
  ...
  /**
   * Creates static methods to wrap functions with return or argument types problematic in jsni
   * (long, date, etc).
   */
  ...
  private void debugJSPassedValues(JExportableMethod method) {
    ...
    for (int i = 0; i < params.length; i++) {
      sw.print("$wnd.alert(\"\"+" + params[i].getExportParameterValue(ARG_PREFIX + i) + ");");
    }
  }

  /**
   * Generate comma separated list of argnames, arg0, ..., arg_n where n = number of parameters of
   * method
   * 
  ...
  /**
   * For each package of sub1.sub2.sub3... we create a chain of objects $wnd.sub1.sub2.sub3
   */
  private void declarePackages(JExportableClassType requestedClassType) {
    // save this namespace to restore later
    sw.print("var pkg = ");
    sw.println("@org.timepedia.exporter.client.ExporterUtil::declarePackage(Ljava/lang/String;)('"
   ...
   */
  private void declareParameters(JExportableMethod method, int arity, boolean includeTypes) {
    JExportableParameter params[] = method.getExportableParameters();
    int numParams = includeTypes || arity < 0 ? params.length : arity;
    for (int i = 0; i < numParams; i++) {
      sw.print((includeTypes ? params[i].getTypeName() : "") + " " + ARG_PREFIX + i);
      if (i < numParams - 1) {
        sw.print(", ");
      }
    }
  ...
  /**
   * Exports a Javascript constructor as $wnd.packageName.classname = function(args) { if(arg0 is
   * GWT type) { this.instance = arg0; } else this.instance = invoke static factory method with args
   * }
   */
  private void exportConstructor(JExportableClassType requestedType)
    ...
    sw.println("if (" + ARG_PREFIX
        + ".length == 1 && __.@org.timepedia.exporter.client.Exporter::isAssignable(*)("
        + ARG_PREFIX + "[0]))");
    sw.println("  g = " + ARG_PREFIX + "[0];");

    // used to hold arity of constructors that have been generated
    HashMap<Integer, JExportableMethod> arity = new HashMap<Integer, JExportableMethod>();
    ...
    for (JExportableMethod constructor : constructors) {
      ...
      if (conflicting != null) {
        logger.log(TreeLogger.ERROR, "Constructor " + conflicting + " with " + numArguments + " "
            + "arguments conflicts with " + constructor + "."
            + "Two constructors may not have identical numbers of " + "arguments.", null);
        throw new UnableToCompleteException();
      }
  ...
  private boolean exportDependentClass(String qualifiedSourceName) throws UnableToCompleteException {
    ...
    if (xType instanceof JExportableArrayType) {
      JExportableType xcompType = ((JExportableArrayType) xType).getComponentType();
  ...
  /**
   * For each exportable field Foo, we generate the following Javascript: $wnd.package.className.Foo
  ...
  /**
   * Export a method If the return type of the method is Exportable, we invoke ClassExporter
   * recursively on this type
   * <p/>
   * For static methods, the Javascript looks like this: $wnd.package.className.staticMethod =
   * function(args) { // body }
   * <p/>
   * for regular methods, it looks like
   * <p/>
   * _.methodName = function(args) { //body }
   * <p/>
   ...
   */
  private void exportMethod(JExportableMethod method, HashMap<String, DispatchTable> dispatchMap)
      throws UnableToCompleteException {
    ...
    if (dt == null) {
      throw new RuntimeException("Unable to find method in dispatchMap table "
          + method.getUnqualifiedExportName());
    }
    if (dt.isOverloaded() && overloadExported.contains(method.getJSQualifiedExportName())) {
      return;
    }
    ...
    String mret =
        retType == null ? "void" : retType.getQualifiedSourceName().replaceAll("^.+\\.", "");
    log("      " + mname + margs + " - " + mret);

    int l = dt.isOverloaded() ? dt.maxArity() : method.getExportableParameters().length;
    if (method.isExportInstanceMethod()) {
      l--;
    }
    for (int i = 0; i < l; i++) {
    ...
    try {
      ...
      if (!dt.isOverloaded()) {
        if (needsExport) {
          sw.print(getGwtToJsWrapper(retType) + "(");
        }
        sw.print((isStatic || method.needsWrapper() ? "@" : "this." + GWT_INSTANCE + ".@")
        ...
        sw.print("@org.timepedia.exporter.client.ExporterUtil::runDispatch("
            ...
            + method.getEnclosingExportType().getQualifiedSourceName() + "::class, "
            + getMethodDispatchIndex(dispatchMap, method.getUnqualifiedExportName())
            + " , arguments, " + method.isInStaticMap() + ", " + method.isVarArgs() + ")[0]");
   ...
   */
  private void exportMethods(JExportableClassType requestedType) throws UnableToCompleteException {
    ...
    for (JExportableMethod method : requestedType.getExportableMethods()) {
      ...
      if (method.isInStaticMap() ? !staticExported.contains(methodName) : !exported
          .contains(methodName)) {
  ...
  /**
   * We create a static factory method public static [typeName] ___create(args) that just invokes
   * the real constructor with the args
   */
  private void exportStaticFactoryConstructor(JExportableConstructor constructor) {
    JExportableClassType consType = (JExportableClassType) constructor.getExportableReturnType();
    String typeName = consType.getQualifiedSourceName();
    sw.print("public static " + typeName + " " + constructor.getStaticFactoryMethodName() + "(");
  ...
  /**
   * Exports a static factory method corresponding to each exportable constructor of the class
   */
  private void exportStaticFactoryConstructors(JExportableClassType requestedType) {
  ...
  private void exportStructuralTypeConstructor(String genName, JExportableClassType requestedType) {
    // export constructor
    sw.println("private " + ExportableTypeOracle.JSO_CLASS + " jso;");
    sw.println();
  ...
  private void exportStructuralTypeField(JStructuralTypeField field) {
    ...
    if (field.isVoidReturn()) {
      ...
      sw.println("org.timepedia.exporter.client.ExporterUtil.setStructuralField(" + "jso, \""
          + field.getName() + "\", arg);");
    } else {
      sw.println(field.getReturnType() + " x = super." + field.getMethodName() + "(arg);");
      sw.println("org.timepedia.exporter.client.ExporterUtil.setStructuralField(" + "jso, '"
          + field.getName() + "', arg);");
      sw.println("return x;");
    }
  ...
  private void exportStructuralTypeImporter(JExportableClassType requestedType) {
  ...
  private void exportStructuralTypeMatchMethod(JExportableClassType requestedType)
      throws UnableToCompleteException {
    ...
    for (JStructuralTypeField field : requestedType.getStructuralTypeFields()) {
      ...
      if (eType == null) {
        logger.log(TreeLogger.ERROR, "Structural type field " + field.getMethodName()
            + " for class " + requestedType.getQualifiedSourceName() + " is not exportable.");
        throw new UnableToCompleteException();
      }
      if (eType instanceof JExportableClassType) {
        ...
        if (cType.needsExport() && cType.isStructuralType()) {
          sw.print("(jso." + field.getName() + " && @"
              + ((JExportableClassType) eType).getQualifiedExporterImplementationName()
              + "::___match(Lcom/google/gwt/core/client/JavaScriptObject;)(jso." + field.getName()
              + ") &&");
   ...
   * <p/>
   * public void export() { Exporter export1 = (Exporter)GWT.create(ExportableDependency1.class)
   * export1.export();
   * <p/>
   * Exporter export2 = (Exporter)GWT.create(ExportableDependency2.class) export2.export();
   * <p/>
   * ... export0(); }
   * 
  ...
  private void genExportMethod(JExportableClassType requestedType,
    ...
    sw.println("ExporterUtil.addExporter(" + requestedType.getQualifiedSourceName()
        + ".class, this);");
  ...
  private void registerDispatchMap(JExportableClassType requestedType,
    ...
    sw.println("\n@org.timepedia.exporter.client.ExporterUtil::registerDispatchMap("
        ...
        + requestedType.getQualifiedSourceName() + "::class," + DispatchTable.toJSON(dispatchMap)
        + ", " + isStatic + ");");
  }
}
